...
CONFIG = {
    ...
    # 层数设置
    # BIF:         1 FAM + 3 TF = 4层
    # BIF-ablation:1 compressed-attn + 3 TF = 4层
    # Baseline:    5 TF层（多一层补参数）
    "bif_n_layers":      4,
    "ablation_n_layers": 4,
    "baseline_n_layers": 5,
    ...
    # 路径
    "device":      "cuda:0",
    "results_dir": "/data/biic/results/bif_phase1",
    "log_file":    "/data/biic/logs/bif_phase1.log",
...
class BaselineModel(nn.Module):
    """
    标准Transformer：标准embedding（无weight tying）+ n_layers层attention
    5层：多一层来补齐BIF省掉的~9.7M embedding参数
    不做weight tying，与BIF/ablation对等
    """
    def __init__(self, vocab_size, d_model=256, n_layers=5, n_heads=4):
        super().__init__()
    ...
    for bs in batch_sizes:
        try:
            ...
            results_path = os.path.join(
                CONFIG["results_dir"], "results_phase1.json"
            )
